...
)
{
    ...
    readFixedString(file, save->saveSignature, FONVSAVE_SIGNATURE_LENGTH, &address, 0);

    if (strcmp(save->saveSignature, FONVSAVE_SIGNATURE) != 0)
    {
        closeFONVSave(save);
    ...
    ok &= readUInt(file, &save->snapshotWidth, &address, 66);
